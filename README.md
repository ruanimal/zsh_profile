...
`zsh_history_cleaner.py` 用于清理和优化 zsh 历史文件。

### 主要功能

1. **清理过期记录**：
   - 多行命令：超过保留时间（默认 30 天）则清理
   - 单行命令：超过 3 倍保留时间（默认 90 天）则清理
   - 无时间戳的记录直接清理

2. **去重处理**：对所有时间的历史记录进行去重，保留最近一条

3. **备份管理**：
   - 每次清理前自动备份原始文件（带时间戳）
   - 维护完整历史备份文件 `zsh_history_full`
   - 自动清理旧备份（保留最近 30 个）

4. **智能多行命令处理**：
   - 区分真正的多行命令和误带反斜杠的单行命令
   - 真正的多行命令：行以 `\` 结尾，且下一行是续行（无时间戳）
   - 误带反斜杠的单行命令：行以 `\` 结尾，但下一行是独立命令（有时间戳）→ 自动去除反斜杠
   - 避免因录入错误导致的命令合并问题
...
```bash
# 预览模式（不实际修改文件）
python3 zsh_history_cleaner.py --dry-run

# 执行清理（保留最近 30 天）
python3 zsh_history_cleaner.py

# 自定义保留时间
python3 zsh_history_cleaner.py --days 60

# 指定历史文件和备份目录
python3 zsh_history_cleaner.py --history-file ~/.zsh_history --backup-dir ~/.zsh_history_backups/
```
